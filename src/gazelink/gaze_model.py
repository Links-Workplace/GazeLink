...
class RegressionModel:
    ...
    def __post_init__(self) -> None:
        ...
        expected = _design_width(self.kind)
        if len(self.coefficients_x) != expected or len(self.coefficients_y) != expected:
            raise ContractValidationError(
                f"{self.kind.value} coefficients must have length {expected}"
            )
        object.__setattr__(
            self,
            "coefficients_x",
    ...
    def predict(self, features: GazeFeatureVector) -> GazePoint:
        design = _design_row(features, self.kind)
        return GazePoint(
            float(design @ np.asarray(self.coefficients_x, dtype=np.float64)),
            float(design @ np.asarray(self.coefficients_y, dtype=np.float64)),
        )
...
def fit_model(
    ...
    *,
    ...
    design = np.vstack([_design_row(vector, kind) for vector in features])
    ...
    if kind is GazeModelKind.LINEAR:
        coefficients_x = np.linalg.lstsq(design, x, rcond=None)[0]
        coefficients_y = np.linalg.lstsq(design, y, rcond=None)[0]
        stored_lambda = 0.0
    else:
        penalty = np.eye(design.shape[1], dtype=np.float64) * regularization
        penalty[0, 0] = 0.0  # The intercept is never regularized.
        system = design.T @ design + penalty
        coefficients_x = np.linalg.solve(system, design.T @ x)
        coefficients_y = np.linalg.solve(system, design.T @ y)
...
def compare_models(
    ...
    *,
    ...
    baseline = cross_validate(rows, geometry, GazeModelKind.LINEAR)
    advanced = cross_validate(
        rows, geometry, GazeModelKind.POLYNOMIAL_RIDGE, ridge_lambda=ridge_lambda
    )
    selected = (
...
def _design_width(kind: GazeModelKind) -> int:
    if kind is GazeModelKind.LINEAR:
        return 1 + GazeFeatureVector.size
    count = len(_POLYNOMIAL_SOURCE_INDICES)
    return 1 + count + (count * (count + 1) // 2)


def _design_row(features: GazeFeatureVector, kind: GazeModelKind) -> NDArray[np.float64]:
    if not isinstance(features, GazeFeatureVector):
        raise ContractValidationError("features must be a GazeFeatureVector")
    if kind is GazeModelKind.LINEAR:
        return np.asarray((1.0, *features.values), dtype=np.float64)
    source = [features.values[index] for index in _POLYNOMIAL_SOURCE_INDICES]
