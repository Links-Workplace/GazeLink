...
from gazelink_core.calibration import schema as S  # noqa: E402
from gazelink_core.domain import common as C  # noqa: E402
from gazelink_core.gaze import gaze_filter as GF  # noqa: E402
from gazelink_core.interaction import gesture as GEST  # noqa: E402
from gazelink_core.paths import PACKAGE_DIR

PROFILE_VERSION = "profile-1"
# How frames reach the network. Every profile and model from before this field
# existed used the library's own 640x480 camera, so ABSENT means exactly that.
# Kept as a literal here (gf_capture defines the same strings) so loading a
# profile never imports the camera stack.
CAPTURE_LIBRARY = "640x480-library"
CAPTURE_HI = "hi-1440x1080-crop-of-1920x1080"
KNOWN_CAPTURES = (CAPTURE_LIBRARY, CAPTURE_HI)
...
class Profile:
    ...
    # Not a version bump on purpose: from_dict rejects any other version, so a
    # bump would make every saved profile unloadable.
    capture_pipeline: str = CAPTURE_LIBRARY
    created_utc: str = ""
...
def resolve_profile(name: str | None) -> Profile:
    """The named profile, or the active one; refuses with a hint when neither exists."""

    profile = load(name) if name else load_active()
    if profile is None:
        known = list_profiles()
        hint = (
...
def model_capture_pipeline(model_dir: Path) -> str:
    """The pipeline a saved model was trained on, from its schema alone.

    Absent means the library path: that is what every model before the field
    was trained on. Read as JSON so the check costs nothing and needs no SVR.
    """

    schema = json.loads((Path(model_dir) / "schema.json").read_text(encoding="utf-8"))
    return str((schema.get("train") or {}).get("capture_pipeline") or CAPTURE_LIBRARY)


def require_capture_match(profile: Profile) -> str:
    """The profile's pipeline, or SystemExit if its model was trained on another.

    A model applied to features from a different capture does not fail; it
    produces a confident point in the wrong place. So this refuses, always,
    and every tool that opens a camera goes through it.
    """

    wanted = profile.capture_pipeline or CAPTURE_LIBRARY
    if wanted not in KNOWN_CAPTURES:
        raise SystemExit(f"profile {profile.name!r} names an unknown capture pipeline {wanted!r}")
    trained = model_capture_pipeline(profile.model_path())
    if trained != wanted:
        raise SystemExit(
            f"profile {profile.name!r} captures with {wanted!r} but its model was trained on "
            f"{trained!r} features. That would place the point confidently in the wrong spot; "
            "nothing was started."
        )
    return wanted
