...
# Device pixels of the screen these sizes are written for (5120x1440). The
# plan fixes button sizes in DEVICE px; converting here, once, keeps the
# layout from silently changing size on a different panel without saying so.
ZONE_SCREEN_DEVICE_PX = (5120, 1440)
# M: 400x150 device px. L (870x288) does not fit three to a row inside the
# 0.40-wide band, and S (200x80) sits under the measured error in every zone,
# so both profiles would score near zero and the comparison would show
# nothing. M is the size at which a difference between models can show up.
ZONE_BUTTON_DEVICE_PX = (400, 150)
ZONE_KEYS = ("UP-L", "UP-C", "UP-R", "MID-L", "MID-C", "MID-R", "LOW-L", "LOW-C", "LOW-R")


def layout_zones(
    size_device_px: tuple[int, int] = ZONE_BUTTON_DEVICE_PX,
    *,
    band: tuple[float, float] = BAND,
    screen_device_px: tuple[int, int] = ZONE_SCREEN_DEVICE_PX,
) -> list[Button]:
    """One button at the centre of each of the nine band zones.

    Row-major, the same numbering as ``gf_targets.band_zone_of``: X is split
    inside the band, Y over the WHOLE height, so the bottom row sits where the
    menu's lower tiles are and the failure being investigated is included.
    """

    width = size_device_px[0] / screen_device_px[0]
    height = size_device_px[1] / screen_device_px[1]
    lo, hi = band
    out = []
    for i, key in enumerate(ZONE_KEYS):
        row, col = divmod(i, 3)
        cx = lo + (col + 0.5) * (hi - lo) / 3
        cy = (row + 0.5) / 3
        out.append(Button(key, cx - width / 2, cy - height / 2, cx + width / 2, cy + height / 2))
    return out


LAYOUTS = {"a": layout_a, "b": layout_b, "zones": layout_zones}
...
def layout_warnings(buttons: list[Button], *, bias_x: float, bias_y: float) -> list[str]:
    ...
    for left in ordered:
        # The horizontal neighbour is the nearest button to the RIGHT that
        # shares some height. In a grid the next button by x is usually in
        # another row, and pairing consecutive ones there compares targets
        # that are not side by side at all.
        beside = [
            b for b in ordered
            if b.x0 >= left.x1 and not (left.y1 < b.y0 or b.y1 < left.y0)
        ]
        if not beside:
            continue
        right = min(beside, key=lambda b: b.x0)
