...
class RightWinkDetector:
    ...
    def cancel(self) -> None:
        """Throw away a wink that is part-way through, and wait for the eye.

        Dropping the QUEUE of fired winks is not enough on its own, and this
        is the half that was missed. At the moment a mode changes the eye may
        already be closed with the hold accumulating: nothing has fired yet,
        so there is nothing in any queue to drop, and the wink lands a moment
        later -- in the new mode, from a closure that began in the old one.

        Clearing the accumulation alone is not enough either. The eye is still
        shut, so the very next frame starts a fresh hold from the same closure
        and one long close is counted twice, once on each side of the change.
        This project has already shipped that exact bug once, with a latch
        that cleared when the rule stopped matching instead of when the eye
        opened: one 2.7 second closure fired four times.

        So the reopen latch is SET here. Nothing accumulates again until the
        eye has actually been seen open, and the ordinary cooldown then
        follows, exactly as it does after a wink that fired.
        """
