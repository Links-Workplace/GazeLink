...
def __getattr__(name: str) -> Any:
    """The library-facing builders moved to the tracking adapter (ARCH-01 E).

    ``build(face_info)``, ``pnp_degrees(face_info)`` and ``_landmarks_xy`` read
    the library's FaceInfo, so they live in ``tracking.face_landmarks`` now.
    The old names still resolve, lazily, for tools and tests; code in the core
    imports the adapter directly.
    """

    moved = {
        "build": "head6_from_face",
        "pnp_degrees": "pnp_from_face",
        "_landmarks_xy": "landmarks_xy",
    }
    if name in moved:
        from gazelink_core.tracking import face_landmarks  # noqa: PLC0415

        return getattr(face_landmarks, moved[name])
